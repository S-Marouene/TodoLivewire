<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex item-center">
            ...
            @if($editTodoId === $todo->id)
                <div>
                    ...
                    @error('editTodoName')
                    <span class="mr-1 text-red-500 text-xs block">{{$message}}</span>
                    @enderror
                </div>
        ...
        <div class="flex items-center space-x-2">
            ...
            <button wire:click="viewDetails({{$todo}})" class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1">
    ...
    @isset($selectedTodo)
        <x-modal name="TodoDetails" title="Todo Details">
            <x-slot:body>
                Id : {{ $selectedTodo->id }}
                ...
                Created at : {{ $selectedTodo->created_at }}
